...
import { registerSleep } from "../../src/tools/v2/sleep.js";
...
describe("compact helper tools", () => {
  ...
  it("uses the scored sleep record for performance, not deep-dive hours-versus-needed", async () => {
    const fixture = (name: string) => JSON.parse(readFileSync(resolve("tests/fixtures", name), "utf8"));
    const get = vi.fn((path: string) => Promise.resolve(
      path === "/developer/v2/activity/sleep" ? fixture("activity_sleep.json") : fixture("deep_dive_sleep.json"),
    ));
    const client = await connect((server) => registerSleep(server, { get } as unknown as WhoopClient));
    const result = await client.callTool({ name: "whoop_sleep", arguments: { date: "2026-05-28" } });
    const output = JSON.parse((result.content as Array<{ text: string }>)[0]!.text);
    expect(result.isError).not.toBe(true);
    expect(output.performance_pct).toBe(83);
  });
});
