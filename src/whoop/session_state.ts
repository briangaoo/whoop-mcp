// Per-MCP-session "catalog consulted" gate. Each server/transport pair gets
// its own instance, so one HTTP client cannot unlock ID-taking tools for a
// different client that has not actually read the relevant catalog.
//
...
export class CatalogGate {
  private readonly consulted = new Set<CatalogName>();

  markConsulted(catalog: CatalogName): void {
    this.consulted.add(catalog);
  }
  ...
  error(
    ...
    if (this.consulted.has(catalog)) return null;
    return {
