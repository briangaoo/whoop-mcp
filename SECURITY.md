...
- **Concurrent connections have isolated catalog gates.** Each MCP transport/session must consult the sports, exercise, or behavior catalog for itself before its ID-taking tools unlock. The Whoop account and token manager remain intentionally shared because one deployment serves one Whoop account.
...
- **Tokens are audience-bound** (RFC 8707). An access token carrying a `resource` claim is rejected unless it matches this server's `/mcp` URL, so a token minted for one deployment can't be replayed against another that happens to share the secret. Legacy tokens without the optional claim remain accepted for compatibility.
