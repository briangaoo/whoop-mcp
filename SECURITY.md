...
- **Concurrent connections share the read cache.** The bounded in-memory GET cache (`src/whoop/client.ts`) is process-global, so two clients connected to one deployment can serve each other's cached reads. Not a security issue (both are the same Whoop account) but worth noting if you fork it to support multiple users. The cache is memory-only (never written to disk or logged), holds only completed **read** responses under an explicit per-endpoint TTL, never retains live HR / state / stress, and is invalidated by any write.
