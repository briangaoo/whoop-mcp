...
- **Concurrent connections share one Whoop account.** The account and token manager are intentionally shared because one deployment serves one Whoop account. Bundled catalogs are local read-only data and do not carry per-connection state.
